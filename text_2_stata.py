...
class text_2_stataCommand(sublime_plugin.TextCommand):
    """ Run selection or current line in Stata through a temporary file
        (for Stata < 13) """
    def run(self, edit):
        settings = sublime.load_settings(settingsfile)
        ...
        if sublime.platform() == "osx":
            ...
            cmd = """osascript -e 'tell application "Finder" to open
                     POSIX file "{0}"' -e 'tell application "{1}" to
                     activate' &""".format(dofile_path, st_name)
            os.system(cmd)
        elif sublime.platform() == "windows":
            vbs_cmd = """CreateObject("WScript.Shell").Run
            \"\""{0}\"" \""{1}\"\"", 4, false """.format(settings.get('stata_name'), dofile_path)
